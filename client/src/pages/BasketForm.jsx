...
const BasketForm = ({ isModalOpen, closeModal, editingBasket }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            let response;
            if (editingBasket && editingBasket._id) {
                response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/baskets/admin/${editingBasket._id}`, {
                    method: "PUT",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify(basketDataToSend),
                });
            } else {
                response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/baskets/admin`, {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify(basketDataToSend),
                });
